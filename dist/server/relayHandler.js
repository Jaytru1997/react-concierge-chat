...
export function createNextRelayHandler(options) {
    ...
    async function GET(request) {
        ...
        if (!isValidSessionId(sessionId)) {
            return Response.json({ success: false, error: 'Invalid session identifier' }, { status: 400, headers: corsHeaders });
        }
        if (mode === 'sse') {
        ...
        if (sessionId === 'all') {
            const allSessions = Array.from(store.sessions.values()).sort((a, b) => b.lastUpdated - a.lastUpdated);
            return Response.json({ success: true, sessions: allSessions }, { headers: corsHeaders });
        }
    ...
    async function POST(request) {
        try {
            ...
            return Response.json({ success: false, error: 'Unknown action type' }, { status: 400, headers: corsHeaders });
        }
        catch (err) {
