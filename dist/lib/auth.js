...
export function parseAuthRoutes(routes) {
    if (!routes)
        return [];
...
export function storeChatUser(user) {
    if (typeof window === 'undefined')
        return;
    try {
...
export function clearStoredChatUser() {
    if (typeof window === 'undefined')
        return;
    try {
...
function extractNameFromPayload(data, fallbackIdentifier) {
    if (!data)
 ...
 */
export async function authenticateWithRoutes(routes, credentials) {
...
export async function checkSessionRoute(route) {
    try {
        const res = await fetch(route, {
            method: 'GET',
